...
This post covers 7 operational excellence patterns embedded in [powerShellScripts](powerShellScripts/), configuration files, and automation setup—the operational layer that makes infrastructure **safe to operate at scale**.
...
Created [Common-AzureCleanupFunctions.psm1](powerShellScripts/Common-AzureCleanupFunctions.psm1) with reusable validation functions:
...
[DeleteSpokeNetworkResources.ps1](powerShellScripts/DeleteSpokeNetworkResources.ps1) requires explicit opt-in for destructive operations:
...
From [DeleteSpokeNetworkResources.ps1](powerShellScripts/DeleteSpokeNetworkResources.ps1):
...
[Setup-GitHooks.ps1](Setup-GitHooks.ps1) automates Git hook installation:
...
[.gitattributes](.gitattributes) enforces line ending consistency:
...
[bicepconfig.json](bicepconfig.json) customizes linting rules:
...
From [Invoke-CleanupOrchestrator.ps1](powerShellScripts/Invoke-CleanupOrchestrator.ps1):
...
1. **Deployment pipelines** ([main.yml](main.yml)) - For long-running Bicep deployments
2. **Cleanup orchestrators** ([Invoke-CleanupOrchestrator.ps1](powerShellScripts/Invoke-CleanupOrchestrator.ps1)) - For sequential cleanup operations
...
*Part of a series on building enterprise-scale Azure infrastructure. This post focuses on operational patterns. See also: [Technical Deep Dive: IaC Patterns](blog-post-technical-deep-dive.md)*

*Written January 2026 after building operational automation for multi-environment Azure platform*
